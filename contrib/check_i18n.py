...
def check_project_name(path):
    invalid_projectname_files = []
    for root, dirs, files in os.walk(path):
        for f in files:
            fname = os.path.join(root, f)
            if (fname.endswith(".py") or fname.endswith("*.md")):
                with open(fname) as f:
                    string = "".join(f.readlines())
                    if "Project Ginger S390x" in string:
                        continue
                    else:
                        invalid_projectname_files.append(fname)
    if invalid_projectname_files:
        print "%s are not having project name %s" % (', '.join(invalid_projectname_files), "Ginger S390x")
        exit(1)
...
def main():
    ...
    for f in sys.argv[1:]:
        ...
        check_project_name(os.path.dirname(f))
